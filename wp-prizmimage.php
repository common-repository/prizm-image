...
class AccusoftImageService {
  ...
  function register_settings( ) {
    ...
    add_settings_field( 'wp_image_service_service_user_id', __( 'Enter your Prizm Image License Key. In order to use Prizm Image, you need to register for a free License Key. You can get your License Key here: <a href="https://www.accusoft.com/products/accusoft-cloud-services/portal" target="_blank">https://www.accusoft.com/products/accusoft-cloud-services/portal</a>', WP_IMAGE_SERVICE_DOMAIN ), 
        ...
        foreach( $attachments as $attachment ) {
         if ($this->file_is_handled_by_image_service($attachment->ID)) { 
          ...
          if ((isset( $original_meta['wp_image_service'] )) 
           ...
           && (stripos($meta['wp_image_service'], 'Error' )  === false ) ) {
          ...
          if ( isset( $meta['sizes'] ) && is_array( $meta['sizes'] ) ) {
            foreach( $meta['sizes'] as $size_name => $size  ) {
              ...
              if ( $original_meta['sizes'][$size_name]['wp_image_service'] == $size['wp_image_service'] && stripos( $meta['sizes'][$size_name]['wp_image_service'], 'PrizmImage error' ) === false ) {
                ...
                echo stripos( $meta['wp_image_service'], 'Erorr:' );
              } else {
  ...
  function do_image_service( $file_path = '') {
    ...
    if ( is_wp_error( $temp_file ) ) {
      @unlink($temp_file);
      if ($temp_file->get_error_message()) {
        return sprintf( __("Error: %s", WP_IMAGE_SERVICE_DOMAIN ), $temp_file->get_error_message());
      }
      else {
        return sprintf( __("Error: %s", WP_IMAGE_SERVICE_DOMAIN ), "Unauthorized Key");
      }
    }
      ...
      if ( !$response || is_wp_error( $response ) ) {
        ...
        if ( 200 != $statusCode ){
          ...
          if ($responseData) {
            ...
            if ($statusCode) {
            $message = 'Server returned status code ' . $statusCode . '->' . $responseBody . "</br>";
            } else {
